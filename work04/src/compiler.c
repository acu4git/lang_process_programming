...
void gen_def_var(char *label, int n) {
  if (label == NULL) {
    error("Parameter label is NULL at gen_define");
    return;
  }

  if (n > 0) {
    // for array
    fprintf(cslfp, "%s\tDS\t%d\n", label, n);
  } else if (n == 0) {
    if (!strncmp(label, "$$", 2)) {
      // for parameter
      fprintf(cslfp, "%s\n", label);
      fprintf(cslfp, "\tDC\t%d\n", n);
    } else if (*label == '$' && strstr(label, "%") != NULL) {
      // for local variable
      fprintf(cslfp, "%s\n", label);
      fprintf(cslfp, "\tDC\t%d\n", n);
    } else {
      // for global variable
      fprintf(cslfp, "%s\tDC\t%d\n", label, n);
    }
  } else {
    error("Can not use negative number at gen_define");
    return;
  }
}
...
void gen_st(char *opr1, char *opr2, char *opr3) {
  if (opr3 == NULL) {
    fprintf(cslfp, "\tST\t%s,%s\n", opr1, opr2);
  } else {
    fprintf(cslfp, "\tST\t%s,%s,%s\n", opr1, opr2, opr3);
  }
}
...
void gen_ld(char *opr1, char *opr2, char *opr3) {
  if (opr3 == NULL) {
    fprintf(cslfp, "\tLD\t%s,%s\n", opr1, opr2);
  } else {
    fprintf(cslfp, "\tLD\t%s,%s,%s\n", opr1, opr2, opr3);
  }
}

void gen_lad(char *opr1, char *opr2, char *opr3) {
  if (opr3 == NULL) {
    fprintf(cslfp, "\tLAD\t%s,%s\n", opr1, opr2);
  } else {
    fprintf(cslfp, "\tLAD\t%s,%s,%s\n", opr1, opr2, opr3);
  }
}

void gen_lad_num(char *opr, int n) {
  fprintf(cslfp, "\tLAD\t%s,%d\n", opr, n);
}
...
void gen_suba(char *opr1, char *opr2, char *opr3) {
  if (opr3 == NULL) {
    ...
    fprintf(cslfp, "\tSUBA\t%s,%s,%s\n", opr1, opr2, opr3);
  }
}

void gen_and(char *opr1, char *opr2, char *opr3) {
  if (opr3 == NULL) {
    fprintf(cslfp, "\tAND\t%s,%s\n", opr1, opr2);
  } else {
    fprintf(cslfp, "\tAND\t%s,%s,%s\n", opr1, opr2, opr3);
  }
}
